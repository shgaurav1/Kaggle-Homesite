...
X_test = []
y = []
final_res = np.zeros([label.shape[0],])
fold = 1
#label = labels[:,i]
print label.shape
#cross_validation
...
for train_indices, test_indices in kf:
    ...
    print 'clf fit'
    #clf = LogisticRegression(penalty='l2', dual=False, tol=0.0001, C=1.0, fit_intercept=True, intercept_scaling=1, class_weight='auto', random_state=None, solver='liblinear', max_iter=500, multi_class='ovr', verbose=1)
    ...
    print Y_pred.shape
    '''
    final_res[test_indices] = clf.predict_proba(X_test)#Y_pred.transpose()
    gc.collect()
